...
class JoinTableTest extends SparkTestCase {
    ...
    test("should be able to merge two tables without deleting the existing key") {
        ...
        val regionTable: RDD[String] = sc.parallelize(List("0,AFRICA,\"regular ,deposits.\""))
        val tableMerger: TableMerger = new TableMerger(nationTable, regionTable)
        val newTable: RDD[String] = tableMerger.merge(primaryKeyIndex = 0, foreignKeyIndex = 2, preserveKey = true)


        assert(newTable.collect().head == "0,ALGERIA,0,\"haggle.,is here\",AFRICA,\"regular ,deposits.\"")
    }
}
